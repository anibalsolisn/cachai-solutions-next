...
export default function HeroSliderEN() {
  ...
  return (
    <section style={{ position: 'relative', width: '100%', height: '100vh', overflow: 'hidden', background: '#1e4a5a' }}>
      {slides.map((s, i) => (
        <video key={i} src={s.video} autoPlay={i === 0} muted loop playsInline
...
function CursorNext({ onNext }) {
  ...
  return (
    <>
      <div onClick={onNext} onMouseMove={e => setPos({ x: e.clientX, y: e.clientY })}
        onMouseEnter={() => setVisible(true)} onMouseLeave={() => setVisible(false)}
        style={{ position: 'absolute', inset: 0, zIndex: 3, cursor: 'none' }} className="hero-click-area" />
      <div style={{ position: 'fixed', left: pos.x, top: pos.y, width: 100, height: 100, transform: 'translate(-50%,-50%)', pointerEvents: 'none', zIndex: 9999, opacity: visible ? 1 : 0, transition: 'opacity .3s' }}>
        <svg viewBox="0 0 100 100" width="100" height="100" style={{ position: 'absolute', inset: 0, animation: 'rotateCursor 8s linear infinite' }}>
