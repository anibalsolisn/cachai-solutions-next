...
const flows = [
  { id: 'welcome', title: 'Welcome Series', desc: 'First impressions that convert. A welcome sequence that introduces your brand and drives the first purchase.', stat: '+35%', statLabel: 'First purchase conversion' },
  { id: 'abandon', title: 'Cart Abandonment', desc: 'Recover the revenue that was already in your pocket. A 3-email sequence that brings back the ones who almost bought.', stat: '+43%', statLabel: 'Cart recovery rate' },
  { id: 'winback', title: 'Win-back', desc: 'Reactivate customers who haven\'t bought in a while. The right offer at the right time.', stat: '28%', statLabel: 'Reactivation rate' },
  { id: 'postpurchase', title: 'Post-Purchase', desc: 'The sale doesn\'t end when they pay. A sequence that generates reviews, upsells and repeat customers.', stat: '3.2x', statLabel: 'LTV vs without sequence' },
  { id: 'campaigns', title: 'Campaigns & Newsletters', desc: 'Regular communication that keeps your brand top-of-mind. Editorial calendars and seasonal campaigns.', stat: '42%', statLabel: 'Average open rate' },
];

export default function EmailENPage() {
    ...
    <main style={{ background: '#fff' }}>
      <section style={{ minHeight: '100vh', background: '#0a1a22', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', padding: '140px 80px 80px', position: 'relative', overflow: 'hidden' }}>
        ...
        <h1 style={{ fontFamily: 'var(--font-playfair)', fontSize: 'clamp(52px, 8vw, 110px)', fontWeight: 400, lineHeight: .95, color: '#fff', maxWidth: 900, marginBottom: 48, position: 'relative' }}>
          The channel with<br />the best <em style={{ fontStyle: 'italic', color: '#7de0f0' }}>ROI</em><br />out there.
        </h1>
        <div style={{ borderTop: '1px solid rgba(255,255,255,.08)', paddingTop: 40, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', gap: 40, flexWrap: 'wrap', position: 'relative' }}>
          <p style={{ fontSize: 16, color: 'rgba(255,255,255,.5)', maxWidth: 480, lineHeight: 1.8 }}>For every $1 invested in email marketing, brands return $42. Automated flows, campaigns and segmentation that generate revenue while you sleep.</p>
          <div style={{ display: 'flex', gap: 32 }}>
            {[{ n: '$42', l: 'ROI per $1' }, { n: '42%', l: 'Open rate' }, { n: '+43%', l: 'Carts recovered' }].map(s => (
      ...
      <section style={{ background: '#fff', padding: '120px 80px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <h2 style={{ fontFamily: 'var(--font-playfair)', fontSize: 'clamp(32px, 3.5vw, 52px)', fontWeight: 400, color: '#0d2030', marginBottom: 64, lineHeight: 1.15 }}>
            Automated revenue<br /><em style={{ fontStyle: 'italic', color: '#2a5f72' }}>while you sleep.</em>
          </h2>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2, background: 'rgba(13,30,42,.06)' }} className="flows-grid">
            <div style={{ background: '#fff', display: 'flex', flexDirection: 'column' }}>
              {flows.map(f => (
                <div key={f.id} onClick={() => setActive(f.id)} style={{ padding: '24px 32px', cursor: 'pointer', borderBottom: '1px solid rgba(13,30,42,.06)', background: active === f.id ? '#f0f8fc' : '#fff', borderLeft: active === f.id ? '3px solid #2a5f72' : '3px solid transparent', transition: 'all .2s', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
      ...
      <section style={{ background: '#1e4a5a', padding: '120px 80px' }}>
        <div style={{ maxWidth: 700, margin: '0 auto', textAlign: 'center' }}>
          <h2 style={{ fontFamily: 'var(--font-playfair)', fontSize: 'clamp(36px, 4vw, 56px)', fontWeight: 400, color: '#fff', lineHeight: 1.1, marginBottom: 24 }}>
            How much revenue<br />are you <em style={{ fontStyle: 'italic', color: '#7de0f0' }}>leaving on the table?</em>
          </h2>
          <p style={{ fontSize: 16, color: 'rgba(255,255,255,.45)', marginBottom: 40, lineHeight: 1.8 }}>We audit your current setup for free and tell you exactly which flows you're missing.</p>
      ...
      <Footer />
      <style>{`@media(max-width:960px){.flows-grid{grid-template-columns:1fr!important}}@media(max-width:600px){section{padding-left:24px!important;padding-right:24px!important}}`}</style>
    </main>
  );
}